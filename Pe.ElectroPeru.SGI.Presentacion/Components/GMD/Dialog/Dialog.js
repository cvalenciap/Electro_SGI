...
Pe.GMD.UI.Web.Components.Dialog.prototype = {
    ...
    noCloseButtonClass: 'no-close-dialog',
    init: function (opts) {        
        opts.id = opts.id == undefined ? newGuid() : opts.id;
        opts.type = opts.type == undefined ? "type-primary" : opts.type; //a.TYPE_PRIMARY,
        opts.size = opts.size == undefined ? "size-normal" : opts.size; //a.SIZE_NORMAL,
        opts.title = opts.title == undefined ? null : opts.title;
        opts.message = opts.message == undefined ? null : opts.message;
        opts.nl2br = opts.nl2br == undefined ? true : opts.nl2br;
        opts.closable = opts.closable == undefined ? true : opts.closable;
        opts.close = opts.close == undefined ? true : opts.close;
        opts.closeByBackdrop = opts.closeByBackdrop == undefined ? false : opts.closeByBackdrop;
        opts.closeByKeyboard = opts.closeByKeyboard == undefined ? true : opts.closeByKeyboard;
        opts.autodestroy = opts.autodestroy == undefined ? true : opts.autodestroy;
        opts.draggable = opts.draggable == undefined ? true : opts.draggable;
        opts.animate = opts.animate == undefined ? false : opts.animate;
        opts.replacebody = true;

        opts.showCloseButton = opts.showCloseButton == undefined ? true : opts.close;
        opts.closeEvent = opts.close == undefined ? null : opts.close;
        this._privateFunction.createDialog.apply(this, [opts]);
        if (opts != null && opts.onClose != null && opts.onClose)
            this.onClose = opts.onClose;
        if (opts != null && opts.onDestroy != null && opts.onDestroy)
    ...
    setSize: function (value) {
        BootstrapDialog.dialogs[this.divDialog.options.id].setSize(value)
    },
    ...
    setContent: function (html) {
        if (this.divDialog) {
            this.divDialog.setMessage(html);
        }
    },
    ...
    _privateFunction: {
        createDialog: function (opts) {
            ...
            //}
            //this.divDialog.dialog(this._privateFunction.getConfig.apply(this, [opts]));
            //this._privateFunction.getConfig.apply(this, [opts]);
            //if (opts.maxHeight != undefined || opts.height != undefined) {
            //    this.divDialog.css('overflow-x', 'hidden');
            //}
        ...
        implementDialogElement: function (opts) {
            var div = new BootstrapDialog(opts); //$('<div />').uniqueId();
            //this.idDivDialog = this.idDivDialog + div.attr('id');
            //div.attr('id', this.idDivDialog);
            //$('body').append(div);
            return div;
